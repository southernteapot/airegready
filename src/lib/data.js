...
export const BLOG_POSTS = [
  ...
  },
  {
    ...
    readTime: '8 min read',
    excerpt:
      "Texas became the third state with a comprehensive AI law on January 1, 2026. A practical walk-through of what TRAIGA prohibits, the NIST AI RMF safe harbor, and the first-in-the-nation regulatory sandbox.",
    featured: false,
  },
  {
...
export const QUESTIONS = [
  {
    ...
    subtitle: 'We\u2019ll use this to prioritize your results.',
    options: [
  ...
  {
    id: 4,
    text: 'Are you building AI into what you offer, or using AI tools built by others?',
    subtitle: 'This changes which obligations apply under the EU AI Act and similar frameworks.',
    options: [
      { label: 'Using off-the-shelf AI tools (ChatGPT, Claude, Copilot, etc.)', value: 'deployer' },
      { label: 'Building AI into the products or services I offer', value: 'provider' },
      { label: 'A mix of both', value: 'both' },
      { label: 'Not using AI yet', value: 'neither' },
    ],
  },
  {
    id: 5,
    ...
    options: [
      ...
      { label: 'Customer support or customer interactions', value: 'customer-support', sensitivity: 1 },
      { label: 'Software development or product features', value: 'software-dev', sensitivity: 1 },
      { label: 'Education or training', value: 'education', sensitivity: 1 },
      { label: 'Hiring or HR decisions', value: 'hiring', sensitivity: 3 },
      ...
      { label: 'Still exploring / not using it yet', value: 'exploring', sensitivity: 0, exclusive: true },
    ],
  },
  {
    id: 6,
    text: 'Where are your customers, users, or end-audience based?',
    subtitle: 'Select all that apply \u2014 this shapes which regulations apply to you.',
    multi: true,
    options: [
      { label: 'U.S. / North America', value: 'us' },
      { label: 'European Union', value: 'eu' },
      { label: 'United Kingdom', value: 'uk' },
      { label: 'Other international markets', value: 'intl' },
      { label: 'Internal use only / no external users', value: 'internal', exclusive: true },
      { label: 'Not sure yet', value: 'unsure', exclusive: true },
    ],
  },
  {
    id: 7,
  ...
  {
    id: 9,
    text: 'Has anyone using AI had training or guidance in the last year?',
    subtitle: 'The EU AI Act requires AI literacy for everyone who interacts with AI \u2014 and it\u2019s in effect now.',
    options: [
      { label: 'No training, people figure it out', value: 'none', score: 0 },
      { label: 'We\u2019ve shared informal tips, docs, or videos', value: 'informal', score: 1 },
      { label: 'Solo \u2014 I pick things up as I go', value: 'solo', score: 1 },
      { label: 'We\u2019ve done structured AI training at least once', value: 'structured', score: 2 },
      { label: 'Ongoing training with regular updates', value: 'ongoing', score: 3 },
    ],
  },
  {
    id: 10,
...
function calcReadiness(answerMap, scoreMap) {
  let score = scoreMap[3] || 0
  const q5 = answerMap[5] || []
  if (q5.includes('software-dev')) score += 1
  if (answerMap[8] === 'important') score += 1
  if (answerMap[4] === 'provider' || answerMap[4] === 'both') score += 1
...
function calcGuardrails(scoreMap) {
  const score = (scoreMap[9] || 0) + (scoreMap[10] || 0) + (scoreMap[11] || 0)

  let label
  if (score <= 1) label = 'Ad Hoc'
  else if (score <= 4) label = 'Minimal Structure'
  else if (score <= 7) label = 'Basic Guardrails'
  else label = 'Managed'

  return { score, max: 11, label }
}

function calcRisk(scoreMap, guardrailScore) {
  const useCaseSens = scoreMap[5] || 0
  const dataSens = scoreMap[7] || 0
  const decisionImpact = scoreMap[8] || 0
  let score = useCaseSens + dataSens + decisionImpact
  if (guardrailScore <= 2 && (dataSens >= 3 || decisionImpact >= 3)) score += 1

  let label
  if (score <= 2) label = 'Low'
  else if (score <= 5) label = 'Moderate'
  else if (score <= 9) label = 'Elevated'
  else label = 'High'

  return { score, max: 15, label }
}

function getPeerContext(answerMap, readiness, guardrails) {
  const role = answerMap[2]
  const level = guardrails.label

  if (role === 'solo-exploring' || role === 'freelance') {
    if (level === 'Ad Hoc' || level === 'Minimal Structure') {
      return 'This is a common starting point for solo operators and freelancers \u2014 not a sign you\u2019re behind.'
    }
    return 'You\u2019re further along than most solo operators. Keep building on what you have.'
  }
  if (role === 'small-team' || role === 'founder') {
    if (level === 'Ad Hoc') {
      return 'Most small teams at early adoption look like this. The gap between use and guardrails is the most common pattern we see.'
    }
    if (level === 'Minimal Structure') {
      return 'Typical for small teams at this stage. Informal guidelines stretch for a while \u2014 formalizing them pays off as usage grows.'
    }
    return 'You\u2019re ahead of most teams your size. Focus on your highest-impact use cases.'
  }
  if (role === 'larger-org') {
    if (level === 'Ad Hoc' || level === 'Minimal Structure') {
      return 'Larger organizations at this stage often have real compliance exposure. Formalizing ownership and review is high priority.'
    }
    return 'You have baseline structure in place. The next step is typically tightening documentation and review.'
  }
  return null
}
...
function getStandoutItems(readiness, guardrails, risk, answerMap, scoreMap) {
  const items = []
  const q5 = answerMap[5] || []
  const q6 = answerMap[6] || []
  const q4 = answerMap[4]

  if (readiness.score >= 3 && guardrails.score <= 3) {
    items.push('You\u2019re actively using AI, but your guardrails haven\u2019t caught up. Basic policies, review, and training would reduce your exposure significantly.')
  }
  ...
  const highSens = q5.filter((v) => ['hiring', 'finance', 'healthcare', 'legal'].includes(v))
  ...
  if ((scoreMap[11] || 0) <= 1 && readiness.score >= 2) {
    items.push('You don\u2019t have a process for choosing AI tools. As usage grows, this becomes a data security and liability gap.')
  }

  if ((scoreMap[9] || 0) === 0 && readiness.score >= 2) {
    items.push('No one using AI has had training. This is an easy, high-leverage gap \u2014 and an EU AI Act requirement if you serve EU users.')
  }

  if (q6.includes('eu') && (q4 === 'provider' || q4 === 'both')) {
    items.push('You build AI and serve EU users \u2014 the EU AI Act treats you as a provider, which carries the strictest obligations.')
  } else if (q6.includes('eu')) {
    items.push('You have EU users, so the EU AI Act applies to how you deploy AI, even if the tools are built by someone else.')
  }

  if (guardrails.score >= 8 && risk.score <= 5) {
    items.push('Your current guardrails are solid relative to your risk level. Keep building on what you have.')
  }

  if ((answerMap[2] === 'solo-exploring' || answerMap[2] === 'freelance') && guardrails.score <= 2) {
...
function buildActionPlan(readiness, guardrails, risk, answerMap, scoreMap) {
  ...
  if ((scoreMap[11] || 0) <= 1 && readiness.score >= 1) {
  ...
  if ((scoreMap[7] || 0) >= 3 && guardrails.score <= 4) {
  ...
  if ((scoreMap[9] || 0) <= 1 && q6.includes('eu')) {
    doNow.push('The EU AI Act requires AI literacy for everyone using AI. Put a simple training plan in place now.')
  } else if ((scoreMap[9] || 0) <= 1 && readiness.score >= 2) {
    next30.push('Run a short AI literacy briefing for everyone who uses AI \u2014 what the tools can\u2019t do, where to be careful, who to ask')
  }

  if (q6.includes('eu') && !doNow.some((a) => a.includes('EU AI Act'))) {
    next30.push('Review EU AI Act applicability for your specific use cases \u2014 provider and deployer obligations differ')
  }

  if ((q4 === 'provider' || q4 === 'both') && risk.score >= 5) {
    next30.push('Document your AI system\u2019s purpose, data sources, and known limitations \u2014 you\u2019ll need this regardless of framework')
  }

  if (q5.includes('hiring')) next30.push('Review AI-assisted hiring processes for bias risk and legal requirements')
  if (q5.includes('healthcare')) next30.push('Audit AI tools that touch protected health information for HIPAA compliance')
  if (q5.includes('finance')) next30.push('Review AI-driven financial decisions for fair lending and disclosure requirements')

  if (q1 === 'policies' && !next30.some((a) => a.includes('policy'))) {
    next30.push('Create a basic acceptable use document and share it with your team or collaborators')
  }
  if (q1 === 'tools-workflows') next30.push('Evaluate your current AI tools against your actual needs \u2014 are you using the right ones?')
  if (q1 === 'risk-compliance') next30.push('Review the regulatory frameworks flagged in this assessment for relevance to your situation')
  ...
  if (guardrails.score <= 4 && readiness.score >= 3) later.push('As AI becomes more central to your work, assign clear ownership for AI policy decisions')
...
function getApplicableFrameworks(answerMap, scoreMap, readiness, guardrails, risk) {
  ...
  const q6 = answerMap[6] || []
  const q4 = answerMap[4]
  const servesEU = q6.includes('eu')
  const servesUK = q6.includes('uk')
  const isProvider = q4 === 'provider' || q4 === 'both'
  const hasHighSens = q5.some((v) => ['hiring', 'finance', 'healthcare', 'legal'].includes(v))
  ...
  if (readiness.score >= 2 || guardrails.score >= 2) {
    frameworks.push({
      ...
      priority: guardrails.score <= 4 ? 'Worth reviewing' : 'Good reference',
    })
  }
  ...
  if (servesEU) {
    frameworks.push({
      ...
      slug: 'eu-ai-act',
      why: isProvider
        ? 'You serve EU users and build AI \u2014 the EU AI Act treats you as a provider, with the strictest obligations. Article 4 AI literacy requirements apply now.'
        : 'You serve EU users, so the EU AI Act applies to you as a deployer. Article 4 AI literacy requirements apply now.',
      priority: 'Worth reviewing',
    })
  } else if (hasHighSens || q5.includes('software-dev') || risk.score >= 6) {
    frameworks.push({
      name: 'EU AI Act',
      slug: 'eu-ai-act',
      why: 'Worth watching even if you don\u2019t currently serve EU users \u2014 the Act has extraterritorial reach and sets the global bar for higher-risk AI.',
      priority: hasHighSens ? 'Worth watching' : 'On your radar',
    })
  }

  if (servesUK || q6.includes('intl')) {
    frameworks.push({
      name: 'Global AI Governance',
      slug: 'global',
      why: 'The UK takes a sector-led approach through existing regulators. Canada, Japan, Brazil, and others are moving in different directions worth tracking.',
      priority: hasHighSens ? 'Worth watching' : 'On your radar',
    })
  }

  if (q5.includes('hiring') || q5.includes('customer-support') || (scoreMap[8] || 0) >= 2) {
    frameworks.push({
      ...
      priority: q5.includes('hiring') ? 'Worth reviewing' : 'Worth watching',
    })
  }

  if (q5.includes('healthcare')) {
    frameworks.push({
  ...
  if (answerMap[2] === 'larger-org' || (scoreMap[8] || 0) >= 4) {
    frameworks.push({
...
function getRecommendedReading(readiness, guardrails, risk, answerMap, scoreMap) {
  ...
  if (q5.some((v) => ['hiring', 'finance', 'healthcare', 'legal'].includes(v)) || q6.includes('eu')) {
  ...
  if ((scoreMap[9] || 0) <= 1) {
    readings.push({ title: 'The EU AI Act\u2019s AI Literacy Requirement: What It Actually Means', href: '/blog/ai-literacy-requirement', reason: 'What AI literacy looks like in practice, not just on paper' })
  }
  if (q5.includes('hiring')) {
  ...
  if (guardrails.score <= 3 && readiness.score >= 2) {
...
export function getAssessmentReport(answers) {
  ...
  return {
    ...
    guardrails,
    risk,
    peerContext: getPeerContext(answerMap, readiness, guardrails),
