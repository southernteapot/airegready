...
function formatReferenceDate(timestamp) {
...
export default function TimelineView({ events }) {
  ...
  const filteredEvents = useMemo(() => {
    return events.filter((ev) => {
      ...
      if (!now) return true
      if (timePeriod === 'past' && ev.parsedTimestamp >= now) return false
      if (timePeriod === 'upcoming' && ev.parsedTimestamp < now) return false
      return true
    })
  }, [activeSources, events, now, timePeriod])

  const groupedWithNow = useMemo(() => {
    const items = []
    let currentYear = null
    let nowInserted = !now
    const showNow = timePeriod === 'all' && Boolean(now)

    for (const ev of filteredEvents) {
      const year = getYear(ev.parsedTimestamp)

      if (showNow && !nowInserted && ev.parsedTimestamp >= now) {
    ...
    return items
  }, [filteredEvents, now, timePeriod])

  const isPast = (timestamp) => (now ? timestamp < now : false)

  return (
    <div className="pt-28 pb-20 px-6">
      <div className="max-w-[860px] mx-auto">
        <Breadcrumb items={[{ label: 'Home', href: '/' }, { label: 'Timeline' }]} />
        ...
        <header className="mb-10">
          ...
          <p className="font-sans text-base text-secondary leading-relaxed max-w-[640px]">
            Major milestones and key dates across AI frameworks and regulations
          </p>
        </header>
          ...
          <div className="relative">
            ...
            <div className="flex flex-col">
              {groupedWithNow.map((item, i) => {
                ...
                const past = isPast(item.parsedTimestamp)
