...
AGENTS.md is the canonical agent-instruction file for this repo; CLAUDE.md imports it via `@AGENTS.md`. When project rules change, update AGENTS.md.
...
- A **free sample** (the Starter Kit's AI Use Inventory, PDF + DOCX) is downloadable at `/free-sample` with files in `public/downloads/`; it is a lead magnet with optional newsletter signup, no email gate. It is the only direct download exposed on the site; keep it a real document from the paid kit, not a teaser, and regenerate the sample files whenever that document changes in the kit.
...
- **Catalog** — `/catalog` is the store and the canonical customer-facing catalog: it opens with both paid kits side by side (Solo Builder first) and a "ten-second comparison" strip; follow-on packets, the in-development preview list, and the roadmap sit below; `/catalog/[slug]` detail pages cover the 13 packages (product JSON-LD, galleries, audience/contents/use-case sections); `/kits` remains as a compatibility route only
...
- **Old prototypes** — preserved under `/preview/home-old`, `/preview/front-page-v2`, `/preview/home-v3`, `/preview/home-v4`; `/front-page-v2` redirects to `/preview/front-page-v2`; new prototypes should be noindex under `/preview`. Preserve preview routes unless Brian asks to delete them; old root prototype URLs should redirect to `/preview` or be removed
...
- The older warm cream/brown editorial style is obsolete except in preserved `/preview` prototypes; revive it only if Brian asks.
...
- Use `support@airegready.com` for public contact and mailto links; do not publish personal email addresses, and never commit personal addresses to the repo
...
## Recent Checkpoint
As of 2026-06-21: the $19 AI Governance Starter Kit and $14 Solo Builder AI Launch Kit are both live on Gumroad, and rebuilt customer ZIPs are attached to the existing Gumroad products. The catalog separates featured follow-ons from an in-development list and roadmap; `/free-sample` offers the AI Use Inventory as a free download; the U.S. state AI laws page has a state-by-state quick-reference table; assessment recommendations route to the real paid kits, the free checklist, and free state guides. A kit quality pass rebuilt all three deliverables (Starter v1.2.0, Solo v1.1.0, Vendor v1.1.0) with bonus filled examples, CSV starter spreadsheets, fixed cross-references, and internal notes removed from customer ZIPs. Remaining useful QA: customer purchase/preview download checks for the two paid kits and optional buyer-update email. The Vendor AI Review Packet is the next product to wire once Brian creates the Gumroad listing. Monetization brainstorm lives in `ideas-for-future-monetization/`. See `agent-memory/TODO.md` and `agent-memory/DECISIONS.md` for current tasks and decision history.
