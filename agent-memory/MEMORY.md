...
- Solo Builder AI Launch Kit public product copy exists at `content/products/side-gig-ai-readiness-kit/overview.md`; the internal folder name remains `side-gig-ai-readiness-kit`. The public preview-only catalog route is `/catalog/solo-builder-ai-launch-kit`; it has no price, checkout, purchase URL, or paid-product schema until exports, pricing, checkout, and delivery QA are completed.
- Solo Builder AI Launch Kit draft Markdown deliverable source files exist under `content/products/side-gig-ai-readiness-kit/deliverables/` as a 14-document source kit plus README; these are not exported customer DOCX/PDF/ZIP files yet.
