...
- The `/regulations/us-state-laws` page has a state-by-state quick-reference table (section id `state-tracker`) summarizing CO, TX, CA, NY, IL, NYC, CT, UT, MD, WA, VA, and NJ, plus a dedicated New York RAISE Act section (id `new-york-raise`); last reviewed 2026-06-19.
...
- Blog post `/blog/anthropic-export-control-vendor-risk` is a June 16, 2026 follow-up to the Fable/Mythos shutdown coverage, focused on AI vendor continuity, model dependency records, fallback planning, and practical buyer review.
- Blog post `/blog/ai-chatbot-enforcement-risk` is a June 22, 2026 State Laws article covering state attorney general scrutiny of chatbot behavior, the reported OpenAI AG probe, Pennsylvania's Character.AI professional-licensing lawsuit, and practical chatbot boundary/safety/vendor-review documentation for founders and buyers.
- Blog post `/blog/missing-gpt-5-6-launch-government-risk` is a June 25, 2026 Federal Policy article covering the rumored but not officially released GPT-5.6 launch, the Fable/Mythos government export-control shutdown as frontier-model launch-risk context, and the lack of public evidence tying the two events causally.
- Blog post `/blog/openai-staggered-gpt-56-release-risk` is a June 25, 2026 Federal Policy article covering Reuters/The Information reporting that the Trump administration asked OpenAI to stagger GPT-5.6 over security concerns, with a practical release-risk-file takeaway for AI buyers and builders.
- New York RAISE Act (frontier AI) is covered at `/blog/new-york-raise-act` and in the `/regulations/us-state-laws` page. Use the FINAL March 2026 chapter-amended version: effective Jan 1, 2027; aligned with California's TFAIA (frontier model = >10^26 ops, large developer = >$500M revenue, 72h/24h incident reporting, critical harm = >50 people or >$1B); NY AG enforcement $1M/$3M plus a DFS disclosure role; no private right of action. The original Dec 2025 bill ($10M/$30M penalties, 100-person threshold) is historical only.
